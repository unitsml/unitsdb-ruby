...
Checks that every unit in the SI digital framework (from references/units.ttl) has an equivalent in the database based on string matching:
...
`--output`, `-o`:: Output file path for updated YAML file (default: spec/fixtures/unitsdb.units.yaml)
`--database`, `-d`:: Path to UnitsDB database (required)
