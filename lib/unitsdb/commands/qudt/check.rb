...
module Unitsdb
  module Commands
    module Qudt
      class Check < Base
        # Constants
        ENTITY_TYPES = %w[units quantities dimensions unit_systems prefixes].freeze
        ...
        def run
          ...
          qudt_data = TtlParser.parse_qudt_vocabularies(source_type: source_type, ttl_dir: ttl_dir)

          # Process entity types
          process_entities(entity_type, qudt_data, direction, output_dir, include_potential)
        end

        private

        # Process all entity types or a specific one
        def process_entities(entity_type, qudt_data, direction, output_dir, include_potential)
          if entity_type && ENTITY_TYPES.include?(entity_type)
            process_entity_type(entity_type, qudt_data, direction, output_dir, include_potential)
          else
            ENTITY_TYPES.each do |type|
              process_entity_type(type, qudt_data, direction, output_dir, include_potential)
            end
          end
        end

        # Process a specific entity type
        def process_entity_type(entity_type, qudt_data, direction, output_dir, include_potential = false)
          puts "\n========== Processing #{entity_type.upcase} References ==========\n"

          db_entities = @db.send(entity_type)
          qudt_entities = TtlParser.get_entities_from_qudt(entity_type, qudt_data)

          puts "Found #{qudt_entities.size} #{entity_type} in QUDT"
          puts "Found #{db_entities.size} #{entity_type} in database"

          check_from_qudt(entity_type, qudt_entities, db_entities, output_dir, include_potential) if %w[from_qudt
                                                                                                        both].include?(direction)

          return unless %w[to_qudt both].include?(direction)

          check_to_qudt(entity_type, qudt_entities, db_entities, output_dir, include_potential)
        end
        ...
        def check_from_qudt(entity_type, qudt_entities, db_entities, output_dir, include_potential = false)
          Formatter.print_direction_header("QUDT → UnitsDB")

          matches, missing_matches, unmatched_qudt = Matcher.match_qudt_to_db(entity_type, qudt_entities, db_entities)

          # Print results
          Formatter.display_qudt_results(entity_type, matches, missing_matches, unmatched_qudt)
        ...
        def check_to_qudt(entity_type, qudt_entities, db_entities, output_dir, include_potential = false)
          Formatter.print_direction_header("UnitsDB → QUDT")

          matches, missing_refs, unmatched_db = Matcher.match_db_to_qudt(entity_type, qudt_entities, db_entities)
