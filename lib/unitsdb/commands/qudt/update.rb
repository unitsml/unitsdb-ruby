    ...
    module Qudt
      class Update < Base
        ...
        def run
          ...
          process_entities(entity_type, qudt_data, output_dir, include_potential)
        end

        private

        # Process all entity types or a specific one
        def process_entities(entity_type, qudt_data, output_dir, include_potential)
          if entity_type && ENTITY_TYPES.include?(entity_type)
            process_entity_type(entity_type, qudt_data, output_dir, include_potential)
          else
            ENTITY_TYPES.each do |type|
              process_entity_type(type, qudt_data, output_dir, include_potential)
            end
          end
        end

        # Process a specific entity type
        def process_entity_type(entity_type, qudt_data, output_dir, include_potential = false)
          ...
          matches, missing_refs, unmatched_db = Matcher.match_db_to_qudt(entity_type, qudt_entities, entity_collection.send(entity_type))
