...
RSpec.describe Unitsdb::Commands::Release do
  ...
  let(:options) { { database: database_path, output_dir: output_dir } }
  ...
  describe "#run" do
    context "with default options (all formats)" do
      it "creates both unified YAML and ZIP archive" do
        ...
        yaml_path = File.join(output_dir, "unitsdb-#{schema_version}.yaml")
        ...
        # Check ZIP archive
        zip_path = File.join(output_dir, "unitsdb-#{schema_version}.zip")
    ...
    context "with yaml format only" do
      let(:options) { { database: database_path, output_dir: output_dir, format: "yaml" } }
    ...
    context "with zip format only" do
      let(:options) { { database: database_path, output_dir: output_dir, format: "zip" } }
    ...
    context "with missing files" do
      ...
      let(:options) { { database: invalid_path, output_dir: output_dir } }
    ...
    context "with inconsistent schema versions" do
      ...
      let(:options) { { database: temp_db_path, output_dir: output_dir } }
